...
import { Image, Text, TouchableOpacity, View, ActivityIndicator, Modal } from 'react-native';
...
const MessageMediaContent = ({
  ...
  const startConversion = async () => {
    ...
    try {
            ...
            if (line.startsWith('data: ')) {
              try {
                ...
                if (data.status === 'start') {
                  setConvertProgress(0);
                } else if (data.status === 'progress' && typeof data.percent === 'number') {
      ...
      xhr.open('POST', `${baseUrl}/api-mobile/auth/chat/chat-ios-convert-mov-to-mp4`);
  ...
  if (item.mediaType === 'video') {
    ...
    const isMovFile = item.isMov && !convertedUrl;
        ...
        >
          <View style={styles.mediaBubbleMetaRow}>
            <View style={styles.mediaTypePill}>
              ...
              <Text style={styles.mediaTypePillText}>{isMovFile ? 'MOV VIDEO' : 'VIDEO'}</Text>
            </View>
          </View>
          ...
          <Text style={styles.mediaBubbleSubLabel}>{isMovFile ? 'Will convert to MP4' : 'HD attachment'}</Text>
        </TouchableOpacity>
        {renderConversionModal()}
      </>
    );
